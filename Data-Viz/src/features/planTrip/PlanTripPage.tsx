...
const PlanTripPage: React.FC = () => {
        ...
        if (index > 0) {
          ...
          markersRef.current.push(
            L.marker([transferStop.latitude, transferStop.longitude], { icon: transferIcon })
              .bindPopup(`<strong>🔄 TRANSFER</strong><br>${transferStop.name}<br>Switch to ${segment.route.name}`)
              .addTo(mapInstanceRef.current!)
          );
        } else {
          ...
          markersRef.current.push(
            L.marker([boardStop.latitude, boardStop.longitude], { icon: boardIcon })
              .bindPopup(`<strong>🚌 BOARD</strong><br>${boardStop.name}<br>${segment.route.name}`)
              .addTo(mapInstanceRef.current!)
          );
        }
      });
  ...
  if (loading) {
    return <div className={styles.center}><h2>🚌 Loading GT Bus Data...</h2></div>;
  }
